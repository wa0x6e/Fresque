...
class Fresque
{
	...
	public function __construct()
	{
		...
		$commandTree = array(
				...
				'test' => array(
						...
						'options' => array('u' => 'username', 'q' => 'queue name', 'i' => 'num', 'n' => 'num', 'l' => 'path')
						)
				);
		...
		if ($this->command === null || !method_exists($this, $this->command))
		{
			...
			$this->output->outputLine('------------------', 'success');
			$this->output->outputLine('Fresque 0.1 by Wan Chen (Kamisama) (2012)');
			...
			{
				$this->output->outputText($name . str_repeat(' ', 10 - strlen($name)) , 'bold');
		...
		else
		{
			if ($helpOption->value === true)
			{
				...
				if(!empty($commandTree[$this->command]['options']))
				{
					...
					foreach ($commandTree[$this->command]['options'] as $name => $arg)
					{
						...
						$this->output->outputLine($o . str_repeat(' ', 15 - strlen($o)) . " --" . $opt->long . str_repeat(' ', 10 - strlen($opt->long)) . " {$opt->longhelp}");
					}
				}
			...
			else
			{
				...
				{
					$this->output->outputLine('Invalid options ' .implode(', ', array_map(function($opt) {return '-' . $opt;}, $unrecognized)) . ' will be ignored', 'warning');
				}
				$this->{$this->command}();
			}
		}
	...
	protected function start($params = null)
	{
		...
		escapeshellarg($this->runtime['Fresque']['lib']) . '; VVERBOSE=true QUEUE=' .
		escapeshellarg($queue) . ' APP_INCLUDE=' .
		escapeshellarg($this->runtime['Fresque']['include']) . '  INTERVAL=' .
		escapeshellarg($interval) . ' REDIS_BACKEND=' .
		escapeshellarg($this->runtime['Redis']['host'] . ':' . $this->runtime['Redis']['port']) . ' COUNT=' . $count .
		 ' php ./resque.php';
		$cmd .= ' >> '. escapeshellarg($this->runtime['Fresque']['log']).' 2>&1" >/dev/null 2>&1 &';
	...
	public function _test($test = false)
	{
		$results = array(
		...
		{
			$results['PHPResque library'] = 'Unable to found PHP Resque library. Check that the path is valid, and directory is readable';
		}
		...
		{
			$results['Redis server'] = 'Unable to connect to Redis server at ' . $this->runtime['Redis']['host'] . ':' . $this->runtime['Redis']['port'];
		}
		...
		$resqueFiles = array(
				'lib'.DIRECTORY_SEPARATOR.'Resque.php',
				'lib'.DIRECTORY_SEPARATOR.'Resque'.DIRECTORY_SEPARATOR.'Stat.php',
				'lib'.DIRECTORY_SEPARATOR.'Resque'.DIRECTORY_SEPARATOR.'Worker.php'
		);
		
		$found = true;
		foreach($resqueFiles as $file)
		{
			if (!file_exists($this->runtime['Fresque']['lib'] . DIRECTORY_SEPARATOR . $file))
			{
				$found = false;
				break;
			}
		...
		{
			foreach($resqueFiles as $file)
				require($this->runtime['Fresque']['lib'] . DIRECTORY_SEPARATOR . $file);
		}
		else {
	...
	private function _loadSettings()
	{
		...
		$config = isset($options['config']) ? $options['config'] : '.'.DIRECTORY_SEPARATOR.'fresque.ini';
		...
		$this->runtime['Fresque']['log'] = isset($options['log']) 	? $options['log'] : $this->settings['Fresque']['log'];
		$this->runtime['Fresque']['lib'] = isset($options['lib']) 	? $options['lib'] : $this->settings['Fresque']['lib'];
		$this->runtime['Fresque']['include'] = isset($options['autoloader']) 	? $options['autoloader'] : $this->settings['Fresque']['include'];
		
		$this->runtime['Default']['user'] = isset($options['user']) ? $options['user'] : $this->settings['Default']['user'];
