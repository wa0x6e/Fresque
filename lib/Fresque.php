            ...
            if ($helpOption->value === true) {
                ...
                if (!empty($commandTree[$this->command]['options'])) {
                    ...
                    foreach ($commandTree[$this->command]['options'] as $name => $arg) {
                        ...
                        $this->output->outputLine($o . str_repeat(' ', 15 - strlen($o)) . " --"
                                . $opt->long . str_repeat(' ', 10 - strlen($opt->long)) . " {$opt->longhelp}");
                    }
                }
                ...
                if (!empty($unrecognized)) {
                    $this->output->outputLine('Invalid options ' .implode(', ', array_map(
                            function($opt) {return '-' . $opt;}, $unrecognized)) . ' will be ignored', 'warning');
                }
                $this->{$this->command}();
            }
        }
